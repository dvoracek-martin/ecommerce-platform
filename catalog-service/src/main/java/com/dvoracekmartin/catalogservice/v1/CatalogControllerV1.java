...
import com.dvoracekmartin.catalogservice.domain.service.MinIOMediaRetriever;
...
public class CatalogControllerV1 {
    ...
    public List<ResponseCatalogItemDTO> getAllProductsAndMixtures() {
        ...
        mediaUploader.uploadBase64Image(BASE_64, "test-image.jpg", "image/jpeg");
        return catalogService.getAllProductsAndMixtures();
    }

    @GetMapping("/media")
    public ResponseEntity<byte[]> getMedia(@RequestParam String objectKey) {
        byte[] mediaData = mediaRetriever.retrieveMedia(objectKey);
    ...
    public List<ResponseEntity<byte[]>> listFolderContents(@RequestParam String folder) {
        List<String> mediaKeys = mediaRetriever.listMediaKeysInFolder(folder);
        List<ResponseEntity<byte[]>> responseEntities = new ArrayList<>();
        for (String mediaKey : mediaKeys) {
            byte[] mediaData = mediaRetriever.retrieveMedia(mediaKey);
            if (mediaData != null) {
    ...
    public List<String> listNamesFolderContents(@RequestParam String folder) {
        List<String> mediaKeys = mediaRetriever.listMediaKeysInFolder(folder);
        log.info("Found {} items in folder '{}'", mediaKeys.size(), folder);
        return mediaKeys;
    }
